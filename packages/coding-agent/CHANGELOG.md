...
### Added
- Added a built-in `pull-request` skill and `/pull-request` command: a bare or drafting request prepares the title and body locally, and publishing a PR, posting comments, or marking a PR ready requires explicit confirmation of the exact target and content; user or project copies still override the bundled versions ([Bwoah My Pi](https://github.com/btimothy-har/bwoah-my-pi)).
