...
### Added
- Added a built-in `pull-request` skill and `/pull-request` command for creating and managing pull requests: a bare or drafting request prepares the title and body locally, publishing or editing a PR requires explicit confirmation of the exact target and content, and marking it ready requires an interactive choice; review handling stays out of scope, and user or project copies still override the bundled versions ([Bwoah My Pi](https://github.com/btimothy-har/bwoah-my-pi)).
