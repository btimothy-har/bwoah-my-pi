...
- Added a structured finish to `/review`: after the review chair synthesizes reviewer results, the new built-in `review_findings` tool saves a session-owned v2 report (artifact or blob) and, in the TUI, opens a findings navigator for per-finding comments — submitted, cancelled, unavailable (RPC/headless), and not-required outcomes are all persisted, and feedback stays advisory without granting edit or publish permission ([Bwoah My Pi](https://github.com/btimothy-har/bwoah-my-pi)).
...
- Added fork binary releases and a matching installer: the Bwoah release workflow builds a self-contained macOS arm64 binary and attaches it to a GitHub Release on this repository tagged `bwoah-v<upstream-version>-<yyyymmdd>-<hhmm>` (UTC), and `scripts/install-bwoah.sh` installs or pins those releases with SHA-256 verification and atomic replacement while `--source` keeps the durable `bun setup` workflow ([Bwoah My Pi](https://github.com/btimothy-har/bwoah-my-pi)).
- Added 16 bundled SQL and Python TTSR rules that warn without interrupting on `.sql`/`.py` edits, including NULL comparisons, `NOT IN` subqueries, bare `JOIN`/`UNION`, `BETWEEN` on timestamps, nondeterministic `ROW_NUMBER()`, derived tables, bare/broad/silent `except`, `raise e`, f-string logging, and one-return wrapper functions. Disable individually via `ttsr.disabledRules` or all bundled rules via `ttsr.builtinRules` ([Bwoah My Pi #20](https://github.com/btimothy-har/bwoah-my-pi/pull/20) by [@btimothy-har](https://github.com/btimothy-har)).
- Added bundled `data-analysis`, `frontend-design`, `python-development`, and `sql` skills; user or project skills with the same name still override them ([Bwoah My Pi #20](https://github.com/btimothy-har/bwoah-my-pi/pull/20) by [@btimothy-har](https://github.com/btimothy-har)).
...
- Changed Bwoah My Pi specialist agents to Markdown frontmatter in each file, with shared review guidance expanded at bundle time; standalone consultations can return prose while `/review` still enforces a strict finding schema ([Bwoah My Pi](https://github.com/btimothy-har/bwoah-my-pi)).
...
- Fixed same-session reloads re-adopting the persistent session home as the execution directory; tools and LSP now remain bound to the active working directory across reloads ([Bwoah My Pi #6](https://github.com/btimothy-har/bwoah-my-pi/pull/6) by [@btimothy-har](https://github.com/btimothy-har)).
