...
- Seven Bwoah review lenses (`conventions-specialist`, `integration-specialist`, `testing-specialist`, `code-clarity-specialist`, `docs-specialist`, `security-specialist`, `data-model-specialist`) are bundled from `src/prompts/agents/specialists/*.md` with YAML frontmatter. Their `{{> specialistReviewMethod}}` partial expands only during bundled loading; copying a source file to `.omp/agents` leaves that placeholder literal. Use `omp agents unpack` for a standalone override, or inline the shared method yourself. The lenses have read-only tools and no nested spawning; read-only child sessions exclude ambient custom/MCP/memory capabilities at runtime. The chair supplies their pinned diff and strict finding/verdict schema for `/review`; standalone consultations have no default output schema.
