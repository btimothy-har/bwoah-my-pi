...
- Seven Bwoah review lenses (`conventions-specialist`, `integration-specialist`, `testing-specialist`, `code-clarity-specialist`, `docs-specialist`, `security-specialist`, `data-model-specialist`) from self-contained `src/prompts/agents/specialists/*.md` files have read-only tools and no nested spawning. The chair supplies their complete pinned diff and a strict finding/verdict schema for `/review`; standalone consultations have no default output schema. File changes are discarded only when task isolation runs.
