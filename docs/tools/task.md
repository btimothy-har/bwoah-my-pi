...
13. Child tool availability: explicit `agent.tools` if provided; auto-add `task` when the agent has `spawns` and depth allows; strip `task` at `task.maxRecursionDepth`; add `hub` only for non-read-only or spawning agents unless tools are restricted; expand `exec` to `eval` + `bash`; strip parent-owned `todo` unless prewalk needs it.
