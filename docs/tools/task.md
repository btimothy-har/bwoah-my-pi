...
| `isolated` | `boolean` | No | With `task.isolation.enabled`, all agents default to isolated clones. Agents declaring `isolation: apply` may pass `false` to run in the parent checkout; discard agents reject `false`. Only available outside plan mode, per item in batch shape or top-level in flat shape. |
...
9. With `task.isolation.enabled`, an omitted `isolated` defaults to true in a Git checkout. An explicit `true` still requires Git; a default spawn outside Git runs unisolated. Plan mode keeps its read-only tools without isolation. `isolation: apply` agent frontmatter captures a baseline, then applies a patch or branch according to settings; absent or `isolation: discard` skips diff capture and deletes its clone without applying changes.
...
11. Non-isolated spawns call `runSubprocess(...)` directly with parent cwd; isolated spawns run inside a PAL clone (`ensureIsolation` → `isoResolve`/`isoStart`). Discard runs keep their yielded result, not file changes. Apply runs capture changes; `task.isolation.apply=false` retains a patch or branch instead of applying it.
...
13. Child tool availability: explicit `agent.tools` if provided; auto-add `task` when `spawns` allows and depth permits; strip it at `task.maxRecursionDepth`; add `hub` only for non-read-only or spawning agents unless restricted. A declared `spawns` policy prevents read-only classification even if listed tools are reads. Read-only agents force `restrictToolNames` at child creation, excluding ambient custom, extension, MCP, memory, and mounted device tools; explicit built-in reads and `yield` remain. Agents declaring `recall`/`reflect` are not classified read-only because restricted sessions do not initialize their backend state. `exec` expands to `eval` + `bash`; parent-owned `todo` is stripped unless prewalk needs it.
...
15. End-of-run lifecycle (keep-alive, in the run finalizer):
    ...
    - ordinary discarded isolated run → one-shot: the clone is deleted at completion; only its result and transcript persist. Explicit kept-alive callers such as `workpool` retain the clone until release;
...
- Isolation is enabled with `task.isolation.enabled`; `isolation.backend` selects `auto`, `apfs`, `btrfs`, `zfs`, `reflink`, `overlayfs`, `projfs`, `block-clone`, or `rcopy`, and the PAL resolves the actual backend with fallback. Isolated children keep the parent's read-only `workspace.related` context but not writable session-added roots. Without Git or with isolation disabled, default discard agents run unisolated with an explicit notice.
- Frontmatter `isolation: apply` enables patch/branch capture and follows `task.isolation.apply` (`true` applies on success; `false` retains artifacts) and `task.isolation.merge` (patch or branch). Absent or `isolation: discard` yields a report without capturing file changes. Neither mode confines absolute paths, processes, or network calls.
- Agent source precedence is first-wins by exact name: project `.omp/agents`; user `.omp/agent/agents`; OMP extension-package `agents/` roots in CLI → project settings → user settings → installed npm/link plugin order; Claude marketplace plugin agents (project before user); then bundled (`scout`, `reviewer`, `security-reviewer`, `task`, `sonic`, `conventions-specialist`, `integration-specialist`, `testing-specialist`, `code-clarity-specialist`, `docs-specialist`, `security-specialist`, `data-model-specialist`, `devils-advocate`).
