...
13. Child tool availability: explicit `agent.tools` if provided; auto-add `task` when `spawns` allows and depth permits; strip it at `task.maxRecursionDepth`; add `hub` only for non-read-only or spawning agents unless restricted. A declared `spawns` policy prevents read-only classification even if the listed tools are reads. Read-only agents force `restrictToolNames` at child creation, excluding ambient custom, extension, MCP, memory, and mounted device tools even when the parent has them; explicitly declared built-in read tools and `yield` remain. `exec` expands to `eval` + `bash`; parent-owned `todo` is stripped unless prewalk needs it.
