...
- **Bundled specialists:** `packages/coding-agent/src/task/specialist-agents.ts`, `src/prompts/agents/specialists/`, the `tools`/`output` lines in `src/prompts/agents/frontmatter.md`, the `...SPECIALIST_AGENT_DEFS` splice in `src/task/agents.ts`, and the mandatory roster in `src/discovery/builtin-skills/code-review/SKILL.md` are fork-owned. Upstream `reviewer`/`security-reviewer` stay as shipped; the seven review lenses carry the finding schema, omit `edit`/`write`, and discard changes only when isolated; `devils-advocate` uses a separate challenge schema and read-only tools. `/review` dispatches the fixed baseline plus conditional `data-model-specialist`.
