...
- **Bundled specialists:** the eight specialist prompt files and shared `review-method.md` partial at the top level of `packages/coding-agent/src/prompts/agents/`, their direct text imports and shared review-method partial registration in `src/task/agents.ts`, the mandatory roster in `src/discovery/builtin-skills/code-review/SKILL.md`, and the three `src/prompts/review*-request.md` templates are fork-owned. Bundled non-worker agents inherit the active model through `@default`; `task` and `sonic` keep `@task`/`@smol`. Upstream `reviewer`/`security-reviewer` retain their descriptions, thinking defaults, bodies, and output schemas; only their default model assignment is fork-owned. The seven specialist lenses declare read-only tools without `spawns` and receive the chair's complete pinned diff plus strict per-call `/review` schema. `src/task/read-only-policy.ts` excludes state-mutating tools, memory-backed readers lacking restricted backend state, and nested spawns; `src/task/structured-subagent.ts` MUST restrict the effective child tool registry for read-only agents so ambient custom/MCP/memory capabilities cannot bypass their declared list. Standalone consultations have no default output schema and return prose plus terminal yield in the same assistant response when no effective schema exists. `devils-advocate` remains read-only. Isolated runs discard file changes; `/review` adds `data-model-specialist` only for applicable data changes.
