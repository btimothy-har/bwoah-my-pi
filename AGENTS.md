...
- **Bundled specialists:** `packages/coding-agent/src/prompts/agents/specialists/*.md`, their direct text imports and shared review-method partial registration in `src/task/agents.ts`, and the mandatory roster in `src/discovery/builtin-skills/code-review/SKILL.md` are fork-owned. Upstream `reviewer`/`security-reviewer` stay as shipped; the seven review lenses omit `edit`/`write` and use the caller's strict `/review` schema, while standalone consultations have no default output schema. `devils-advocate` has read-only tools and no default schema. Isolated runs discard their file changes; `/review` adds `data-model-specialist` only for applicable data changes.
